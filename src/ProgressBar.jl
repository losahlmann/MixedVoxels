...
function next!(p::Progress, logmsg::String = "")
	t = time()
	p.counter += 1
	if p.counter >= p.n
		if p.printed
			...
			printover(msg, logmsg, p.color)
			println()
		end
		return
	end

	if t > p.tlast+p.dt || logmsg != ""
		percentage_complete = 100.0 * p.counter / p.n
		bar = barstring(p.barlen, percentage_complete)
...
printover(s::String, logmsg::String, color::Symbol = :color_normal) = printover(STDOUT, s, logmsg, color)
